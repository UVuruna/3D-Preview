...
**Generate the descriptor when the shape is the point, not the content.** Five Stations is one CHOREOGRAPHY that happens to apply to any of the cube's 13 axes; hand-authoring 13 near-identical JSON blocks would be exactly the enumeration Compute, Don't Generate (rules/CODE.md) forbids. [Cinematics](preview3d/__about/cinematics.md) computes the descriptor from the model and the chosen axis instead, and ships one baked instance so the shipped scene is still ordinary data like every other.
