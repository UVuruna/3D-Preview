...
- **Defaults-as-config:** every tunable lives in an exported `*_DEFAULTS` object at the top of its module (No Hardcoded Values (rules/CODE.md)), overridable per instance/spec.
- **One palette table for all shapes:** the six pole colours live once in `shared/spec.json` and dress both the axes gizmo and the cube's faces — a colour belongs to a DIRECTION, not to the shape pointing that way (Compute, Don't Generate (rules/CODE.md)). Every colour beyond those six is COMPUTED from them (`axiscolors.js`); a hardcoded derived hex fails `tests/test_axis_colors.py`.
