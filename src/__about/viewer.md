...
- [Ground Grid](grid.md) — rebuilt on every content swap and orientation change
- [Keyboard Control](keyboard.md) — attached in the constructor when `options.keyboard` is true
- [Model View](modelview.md) — `showModel`, `setModelView`, orientation and switcher plumbing
- [Orientations](orientations.md) — `stepOrientation`
- [Switcher](switcher.md) — `setSwitcher`
- `views.js` (Trivial — view presets and the cycle order, see [Source (folder)](../___src.md)'s file table)
...
- [Preview3D Widget (Python mirror)](../../preview3d/__about/widget.md) — every Python method maps onto one method here
...
- `showModel(model, view)` / `setModelView(name)` / `modelViews()`: the model layer, via `modelview.js`
...
#### Parts
`listParts()`, `setPartVisible(path, visible)`, `setPartOpacity(path, alpha)`, `setPartPosition(path, position)`, `setPartStroke(path, progress)`, `showOnly(groupPath, childName)`, `removePart(path)` — see [Parts](parts.md) and [Making Models](../../MODELS.md). `setPartPosition` and `setPartStroke` are M3's additions: an absolute position (the mirror of `Node.position` in the LIGHT renderer), and 0..1 of a line part's own length drawn from its start toward its end — the Hexagram X-ray triangles' "draw themselves" effect and the Five Stations beads' slide, both `part.position` / `part.strokeProgress` in [Animation Scenes](../../SCENES.md).

#### Orientation & Switcher
- `setOrientation(identifier)` / `stepOrientation(step)`: snap the content to one of the cube's 24 orientations; deliberately does NOT re-frame
- `setSwitcher(register, reading)` / `switcherState()`: which vocabulary and readings are lit, see [Switcher](switcher.md)
...
- `snapTo(direction)`: look down an arbitrary token/vector and re-frame — what the seven presets cannot express (the four body diagonals)
...
- `fitView()` / `resetView()`: frame the content (algorithm in the [flow diagram](../__flow/viewer.md))
- `cameraState()`: azimuth, elevation, distance, view, projection, grid state, content version, orientation, model view
...
The [Timeline](animation.md) resolves values; the viewer applies them, and that split is what lets the identical descriptor drive the LIGHT renderer too. The apply loop is in the [flow diagram](../__flow/viewer.md).
